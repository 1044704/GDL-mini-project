...
if __name__ == '__main__':
    ...
    best = []
    energies = np.zeros(opt["nlayers"]+1)
    for split in range(n_splits):
        best_test_acc, model = train_GNN(opt,split)
        best.append(best_test_acc)
        
        dirichlet_energies = np.array(model.energies)
        energies += dirichlet_energies / n_splits
...
# GCN with res [13331.975781250001, 13879.48984375, 17118.810351562497, 23160.580126953126, 32047.07647705078, 43352.66987304687]
